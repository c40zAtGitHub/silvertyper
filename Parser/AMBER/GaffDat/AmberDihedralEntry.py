...
class AmberDihedralEntry(AmberEntry,DihedralLineParser):
    ...
    @classmethod
    def fromString(cls,entryStrings):
        ...
        if isinstance(entryStrings,str):
            #single line input
            dtype,d,Vn,gamma,n,desp = processSingleLine(entryStrings)
            obj = cls(dtype,d,Vn,gamma,n,description = desp)
        elif isinstance(entryStrings,list):
            #multi line input
            dtype,d,Vn,gamma,n,desp = processSingleLine(entryStrings[0])
            obj = cls(dtype,d,Vn,gamma,n,description = desp)
            for line in entryStrings[1:]:
